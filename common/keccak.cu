...
*************************************************/
__device__ void store64(uint8_t x[8], uint64_t u){
  unsigned int i;
...
**************************************************/
__device__ uint64_t load64(const uint8_t x[8]) {
  unsigned int i;
  uint64_t r = 0;
...
/*************************************************
* Name:        KeccakF1600_StatePermuteDev
*
                          ...
                          uint8_t p)
{
  ...
  while(mlen >= r) {
    ...
    KeccakF1600StatePermuteDev(state);
    mlen -= r;
    m += r;
  }
